...
const ROLE_PERMISSIONS: Record<TenantRole, Permission[]> = {
    ...
    [TenantRole.ADMIN]: [
        ...
        PERMISSIONS.VIEW_RESOLUTIONS,
        PERMISSIONS.EDIT_RESOLUTIONS,
        PERMISSIONS.DELETE_RESOLUTIONS,
        PERMISSIONS.COMMENT_RESOLUTIONS,
        PERMISSIONS.ASSIGN_RESOLUTIONS
    ],
...
export class TenantPermissionService {
    ...
    async checkPermission(
        ...
        try {
            ...
            logger.info('🔍 TenantPermissionService.checkPermission called', {
                userId,
                tenantId,
                permission,
                resourceId,
            ...
            logger.info('👤 Tenant membership retrieved', {
                userId,
                tenantId,
                permission,
            ...
            if (!membership) {
                logger.warn('❌ User is not a member of this tenant', { userId, tenantId });
                return {
                    granted: false,
            ...
            if (!membership.isActive) {
                logger.warn('❌ User membership is inactive', { 
                    userId, 
                    tenantId, 
            ...
            const roleHasPermission = this.roleHasPermission(membership.role, permission);
            logger.info('🎭 Role-based permission check', {
                userId,
                tenantId,
                permission,
                userRole: membership.role,
                roleHasPermission,
                availableRolePermissions: ROLE_PERMISSIONS[membership.role]?.length || 0
            });

            if (roleHasPermission) {
                logger.info('✅ Permission granted by role', {
                    userId,
                    tenantId,
                    permission,
                    role: membership.role
            ...
            const featurePermissions = membership.featurePermissions || [];
            logger.info('🔧 Feature permission check', {
                userId,
                tenantId,
                permission,
                featurePermissions,
                featurePermissionsCount: featurePermissions.length
            });

            if (featurePermissions.some(fp =>
                this.featurePermissionMatches(fp, permission)
            )) {
                logger.info('✅ Permission granted by feature override', {
                    userId,
                    tenantId,
                    permission,
                    featurePermissions
            ...
            if (resourceId && resourceType) {
                ...
                if (resourcePermission.granted) {
                    logger.info('✅ Permission granted by resource access', {
                        userId,
                        tenantId,
                        permission,
                        resourceId,
            ...
            logger.warn('❌ Permission denied - no matching rule found', {
                userId,
                tenantId,
                permission,
                userRole: membership.role,
                checkedSources: ['role', 'feature', resourceId && resourceType ? 'resource' : null].filter(Boolean)
            });
            ...
            logger.error('❌ Error checking permission in TenantPermissionService', {
    ...
    private roleHasPermission(role: TenantRole, permission: Permission): boolean {
        ...
        logger.info('🎭 Role permission check details', {
            role,
            permission,
            hasPermission,
            rolePermissionsCount: rolePermissions.length,
            rolePermissions: rolePermissions.slice(0, 10), // Log first 10 permissions to avoid spam
            allRolePermissions: rolePermissions.length <= 10 ? rolePermissions : `${rolePermissions.length} total permissions`
        });
    ...
    private async getTenantMembership(
        ...
        logger.info('🔍 getTenantMembership called', {
            userId,
            tenantId,
            cacheKey
        });
        ...
        if (this.membershipCache.has(cacheKey)) {
            ...
            if (now < expiry) {
                const cachedMembership = this.membershipCache.get(cacheKey) || null;
                logger.info('📋 Using cached membership', {
                    userId,
                    tenantId,
                    cacheKey,
                    cachedMembership: cachedMembership ? {
                        ...
                        isActive: cachedMembership.isActive
                    } : null
                });
                return cachedMembership;
            } else {
                logger.info('⏰ Cache expired, fetching fresh data', { userId, tenantId, expiry, now });
            }
        } else {
            logger.info('🆕 No cache entry found, fetching from database', { userId, tenantId });
        }

        try {
            // Fetch from database
            logger.info('🔍 Querying tenant_memberships collection', {
                userId,
                tenantId,
                query: {
                    userId: userId,
            ...
            logger.info('📊 Database query results', {
                userId,
                tenantId,
                queryEmpty: membershipQuery.empty,
                docsCount: membershipQuery.docs.length,
                querySize: membershipQuery.size
            });

            if (membershipQuery.empty) {
                logger.warn('❌ No tenant membership found in database', {
                    userId,
                    tenantId,
            ...
            logger.info('📄 Raw membership document data', {
                userId,
                tenantId,
                docId: membershipDoc.id,
                rawData: membershipData,
                dataKeys: Object.keys(membershipData)
            });
            ...
            logger.info('✅ Processed membership object', {
                userId,
                tenantId,
                membership: {
                    id: membership.id,
            ...
            logger.info('💾 Membership cached successfully', {
                ...
                tenantId,
                cacheKey,
                cacheExpiry: now + this.CACHE_TTL
            });

            return membership;
        } catch (error) {
            logger.error('❌ Error fetching tenant membership from database', {
                userId,
                tenantId,
                error: error instanceof Error ? error.message : String(error),
                stack: error instanceof Error ? error.stack : undefined
