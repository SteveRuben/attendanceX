...
const STEPS = [
  ...
  { id: 'visual', title: 'Visuel', icon: Palette },
  { id: 'event', title: 'Événement', icon: Calendar },
  { id: 'location', title: 'Lieu', icon: MapPin }
]

// Liste des pays avec codes ISO2 (version réduite pour l'exemple)
const countries = [
  { code: 'FR', name: 'France' },
...
export const ProjectCreationWizard: React.FC<ProjectCreationWizardProps> = ({
  ...
  const { timezone: tenantTimezone } = useDateTimeFormat()
  ...
  const [data, setData] = useState<WizardData>({
    ...
    description: '',
    colorTheme: '#3B82F6',
    timezone: tenantTimezone || 'Europe/Paris',
    eventDetails: {
      type: 'conference',
      startDate: '',
      endDate: '',
  ...
  const updateAddress = (field: string, value: any) => {
    setData(prev => ({
      ...
      eventDetails: {
        ...
        location: {
          ...prev.eventDetails.location,
          address: prev.eventDetails.location.address ? {
            ...prev.eventDetails.location.address,
            [field]: value
          } : {
            street: field === 'street' ? value : '',
            city: field === 'city' ? value : '',
            country: field === 'country' ? value : 'FR'
          }
        }
      }
    }))
  ...
  const canGoNext = () => {
    switch (currentStep) {
      ...
      case 4: // Location
        if (data.eventDetails.location.type === 'physical' || data.eventDetails.location.type === 'hybrid') {
          return data.eventDetails.location.address?.street && 
                 data.eventDetails.location.address?.city && 
                 data.eventDetails.location.address?.country
        }
        if (data.eventDetails.location.type === 'virtual' || data.eventDetails.location.type === 'hybrid') {
          return !!data.eventDetails.location.virtualUrl
        }
        return true
      default:
        return false
              ...
              <CardContent className="pt-6 space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Décrivez votre projet et ses objectifs..."
                    rows={4}
        ...
        return (
          <div className="space-y-6">
            <div className="text-center">
              ...
              <p className="text-muted-foreground">
                Configurez les informations principales de votre événement
              </p>
            </div>
              ...
              <CardContent className="pt-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <DateTimePicker
                    ...
                    onChange={(value) => updateEventDetails('startDate', value)}
                    required
                  />
                  ...
                  <DateTimePicker
                    ...
                    onChange={(value) => updateEventDetails('endDate', value)}
                    required
                  />
                </div>
            ...
            <Card>
              <CardContent className="pt-6 space-y-4">
                <div>
                  ...
                  <Select
                    id="locationType"
                    value={data.eventDetails.location.type}
                ...
                <div>
                  ...
                  <Input
                    id="locationName"
                    value={data.eventDetails.location.name}
                    onChange={(e) => updateLocation('name', e.target.value)}
                    placeholder="Ex: Salle de conférence A, Zoom Meeting"
                  />
                </div>
                
                {(data.eventDetails.location.type === 'physical' || data.eventDetails.location.type === 'hybrid') && (
                  <div className="space-y-4">
                    <h3 className="font-medium">Adresse physique</h3>
                    
                    <div>
                      <Label htmlFor="street">Rue *</Label>
                      <Input
                        id="street"
                        value={data.eventDetails.location.address?.street || ''}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <Input
                          id="city"
                          value={data.eventDetails.location.address?.city || ''}
                      ...
                      <div>
                        ...
                        <Select
                          id="country"
                          value={data.eventDetails.location.address?.country || 'FR'}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <Input
                          id="postalCode"
                          value={data.eventDetails.location.address?.postalCode || ''}
                      ...
                      <div>
                        ...
                        <Input
                          id="province"
                          value={data.eventDetails.location.address?.province || ''}
                          onChange={(e) => updateAddress('province', e.target.value)}
                          placeholder="Province ou état (optionnel)"
                        />
                      </div>
                    </div>
                  </div>
                ...
                {(data.eventDetails.location.type === 'virtual' || data.eventDetails.location.type === 'hybrid') && (
                  <div>
                    <Label htmlFor="virtualUrl">URL de réunion virtuelle</Label>
                    <Input
                      id="virtualUrl"
                      value={data.eventDetails.location.virtualUrl || ''}
